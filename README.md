...
A small browser game about taking the scenic route. Drive a low-poly Chevrolet Astra downhill through a procedural landscape, with a gentle chase camera and forgiving drift. Switch between eight customizable cars whenever you like, including the 1969 Camaro SS and 1967 Mustang fastback. Built with TypeScript, Three.js, and Vite. Driving and personal scores work offline; a Cloudflare Worker and D1 database provide a worldwide leaderboard. No downloaded 3D assets required.
...
**280 km/h** top speed. Existing saved settings are respected; **Advanced settings
...
The welcome menu initially focuses **Easy drive**. Use the arrow keys to switch between the two choices, then Enter (or Space) to launch the focused one. A soft sage glow follows the focused choice; reduced-motion preferences keep a steady highlight instead. Tab still reaches the other controls, and arrow keys in settings keep their normal behavior.
...
The HUD shows points, the next-pass streak multiplier, lives, near-miss count, and small bonus/penalty feedback. In production, **click the points** or **View scoreboard** while paused to browse the worldwide top ten. The modal freezes gameplay and contains keyboard focus. At game over the board is already in the results, alongside earned points, shoulder deductions, near misses, best streak, distance and drift statistics. Completed runs save once; restarting, reloading or switching modes abandons an unfinished run. Records include car(s), starting seed, date, distance, moving average/top speed and shoulder time. Metric/imperial affects display only: canonical scoring uses km/h, and regression replays produce identical physics, near-miss awards, drift points, penalties and records even when switching units mid-run.

One worldwide leaderboard retains the ten best named runs across every game version and both **Standard** and **Custom** setups. The setup labels describe how a run was driven; they do not split rankings. **Standard** uses the canonical 280 km/h cap, 10 m road, curve tightness 1, curve length 1×, grade 0.09 and drift 0.55; any starting car/seed and cosmetic settings are allowed. Custom or environment-overridden difficulty starts on **Custom**. Changing difficulty, the route seed, or cars during a run permanently tags that run Custom, even if settings are restored. Scenery, paint and units are cosmetic; a scenery recipe that also changes road geometry does count as a difficulty change. Personal records use `chillhill.scores` in this browser; named worldwide entries are stored in Cloudflare D1 and shared across devices. Blocked local storage falls back to a bounded session-only board with a visible warning. Invalid records are rejected, IDs deduplicated, and ties sort by near misses then earlier completion. The server supplies completion time for worldwide tie ordering.

`src/config/scoring.ts` holds all scoring constants, canonical driving settings and historical version metadata. `src/game/scoring.ts` is pure fixed-step scoring; `src/scoreboard.ts` handles classification, completed records and bounded private storage (no local leaderboard UI). Historical version metadata never filters ranking or qualification. Keep the permanent storage key and preserve compatibility with existing record fields when changing scoring or driving physics. `npm run test:scoreboard` checks development visibility, completed runs, categories, units and responsive summaries; `npm run test:online-scoreboard` covers live-view focus, name entry, rank races and offline states using mocked requests. `npm run test:run-leaderboard` runs real production gameplay through local Cloudflare with intercepted APIs; it never publishes test scores. `npm run test:challenge` exercises real point awards and game-over saving.
...
## Replays and shareable videos

Choose the **clapperboard** in the top-right driving toolbar, or pause with **Esc / P**
and choose **Replay & save video**. The same button is available after a Drift king run.
Both driving modes automatically record their most recent **10 minutes of active play**,
capturing motion at **60 samples per second**.
Playback has pause, seeking, ½×/1×/2× speed, and recorded/chase/front camera choices.
Your live car, traffic, world clock, lives and score stay paused; **Back to drive** returns
to that run's pause card. Playback never submits leaderboard scores.
An icon toggle selects portrait/landscape; quality and clip length are always visible above **Save video**.

- **Save video** exports the last 30 seconds by default; choose the last **10 or 15 seconds**,
  the last minute, or the full replay. A separate worker renders and encodes the video as fast as
  your device allows, without playing it in the foreground. Choose **Back to drive**
  and resume playing while it exports. A progress card stays available with **Cancel**
  and, when ready, automatically attempts the download. **Download video** remains
  available if the browser needs another click. The movie stays at normal speed regardless of
  export speed. Default quality renders silent **1080p at 60 fps and 12 Mbps**
  (about 45 MB per 30 seconds). **Portrait · 9:16** is the default for social clips
  at 1080×1920; choose **Landscape · 16:9** for 1920×1080. The **720p** option uses
  720p at 30 fps and 5 Mbps. Higher quality takes more time and produces larger files.
  MP4/H.264 is preferred, with WebM/VP8 as a codec fallback. Resizing or losing focus does
  not cancel it. Keep the page open: browsers can slow or suspend background tabs,
  and closing or refreshing the page stops the job. Export and driving share the GPU.
- **Record in real time** is an explicit fallback for browsers without worker WebGL
  or WebCodecs encoding. It plays the clip on screen and records the visible canvas;
  the selected quality requests 60 or 30 fps and its bitrate, but resolution, format
  and actual cadence follow the visible canvas. Keep the tab active and window size
  steady. This fallback automatically downloads
  the video, with a **Download video** link if the download does not start.
- Replay data stays inside the game; there are no replay-file import or export controls.
- Recording lives in memory: **save a video before restarting, changing driving modes, or
  refreshing**. Older portions of drives longer than ten minutes roll out of the buffer.
  Driving from before this feature was loaded cannot be recovered. Replays work without
  browser storage or an online account.

Particles and lighting transitions are recreated during playback, so their exact
appearance can differ when seeking.
Run `node scripts/replay-check.mjs` against local Vite (`TEST_URL` overrides its URL) to
check toolbar/pause entry, playback, the real-time fallback, mobile layout and live-run isolation.
Run `node scripts/replay-export-check.mjs` to verify background export and automatic downloads,
movie duration, distinct adjacent 60 fps frames, both quality levels, portrait output,
cancellation and returning to driving.

Background export uses the same `GameScene` on a worker-owned `OffscreenCanvas`,
samples replay poses at explicit 1/60-second intervals (1/30 for 720p), and passes timestamped frames
to WebCodecs through Mediabunny's `CanvasSource`. The library handles encoder
backpressure and MP4/WebM muxing. It loads with the export worker only. One job runs
at a time with a 200 MB encoded-data cap; use shorter clips or 720p for long
replays. Canceling terminates the worker and frees
its renderer and encoder. There is no video upload or server rendering cost. Reliable
export after the browser closes would require a separate server job and storage.
...
Road stretches are planned independently of the 180 m loading chunks. The balanced default allocates 45% of forward route distance to short/medium bends, 30% to long sweeps, 15% to flowing S-curves, and 10% to straights. These are distance budgets, not selection probabilities, so longer sections cannot silently dominate the drive. Their order, duration, direction, strength and asymmetry vary by seed and absolute position. There are at most two consecutive twisty sections and no consecutive long sweeps, including across planning boundaries. Long arcs remain uninterrupted; S-curves are their own smooth, opposite-lobed section rather than noise layered on every curve. Smooth easements and slow lateral wandering preserve position, heading and curvature at joins. Worker generation, front-camera loading and revisiting a section reproduce the same road regardless of loading order, and the slope stays downhill.

Advanced settings → **Curve mix** moves from Twisty (15% long sweeps) through Balanced (30%) to Sweeping (55%); short/medium bends take the remaining share while S-curves and straights retain their breathing-room budgets. **Curve length** lengthens bends from 0.6× to 1.6× without stretching straights; **Curve tightness** adjusts bend strength separately. Defaults are mix 0.5 and length 1×, including older saves that lack these fields. Set `VITE_CURVE_MIX` and `VITE_CURVE_LENGTH` for environment defaults; scene recipes and exports carry both `curveMix` and `curveLength`. Changing road shape during Drift king safely recenters the car and marks the run Custom. A fixed 32-plan cache plus two 64-piece spline caches keep route memory bounded. `src/game/road-stretches.ts` plans the stretches in physical world metres, `src/game/route-spline.ts` provides seeded slow wandering and slope variation, and `src/game/route.ts` combines them with road-clearance bounds. Existing saved seeds now produce the new generator's routes, not the former layouts.

Straights normally run **80–160 m**, with a **220 m maximum uninterrupted near-flat run**, including gentle curve tails and inflections. Consecutive straights are forbidden, including across planning boundaries. Larger curve-length settings distribute the same 10% distance budget into more short rests instead of scaling the rests themselves. Neighbouring curve transitions reserve part of the 220 m allowance; the remaining budget accounts for the actual path length on the steepest supported grade. Very long sweep envelopes are limited to 2,400 m to keep their shallow transitions/inflections within the cap while retaining their sustained turning cores. `straightRoadLimits` in `src/config/road-shape.ts` centralizes the metre limits and near-flat curvature threshold (0.00004 rad/m). These limits cover the supported tightness range 0.2–1.7, independently of displayed units. `tests/straight-length.test.ts` checks physical lengths, all mix/length slider steps, nearly-flat transitions, distant boundaries and deterministic regeneration.
