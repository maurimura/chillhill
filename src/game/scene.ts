...
import { CityScenery } from './city';
import { LakeWater, lakeGeometries } from './lakes';
import { installTreeFade, TreeOcclusion } from './tree-occlusion';
import type { ChallengeState } from './challenge';
import { TrafficVisuals } from './traffic-visuals';
...
export class GameScene {
  ...
  private lakeWater = new LakeWater();
  private city = new CityScenery();
  private traffic = new TrafficVisuals();
  readonly collisionDebug = new CollisionDebug();
  private atmosphereTransition = 0;
  ...
  private wheelContact = new THREE.Vector3();
  ...
  private terrainPrefetch = new TerrainPrefetch();
  private preparedForChunk?: number;
  private treeOcclusion = new Map<number, TreeOcclusion>();
  private fadedTrees = 0;
  ...
  private mesaGeometry: THREE.BufferGeometry = mesaGeometry(0);
  ...
  applySettings(settings: Settings, gradual = false) {
    this.atmosphereTransition = gradual ? 15 : 0;
    ...
    if (worldChanged) {
      this.terrainPrefetch.reset();
      this.preparedForChunk = undefined;
    ...
    for (const [name, color] of Object.entries({
      ...
      if (this.materials[name]) this.materials[name].color.set(color);
        ...
        if (name === 'tree' || name === 'treeLight' || name === 'trunk')
          installTreeFade(this.materials[name]);
      }
    }
    ...
    if (shapeChanged) {
      ...
      this.mesaGeometry = mesaGeometry(settings.roundness);
  ...
  private buildChunk(id: number) {
    ...
    const desert = settings.landscape === 'desert';
    const forest = settings.landscape === 'forest';
    const city = settings.landscape === 'city';
    const lakes = settings.landscape === 'lakes';
    const basins = lakes ? nearbyLakes(start, CHUNK, settings) : [];
    const underwater = (s: number, x: number, y: number) =>
      basins.some(
        (lake) =>
          y < lake.elevation + 0.3 &&
          ((x - lake.centerX) / lake.radiusX) ** 2 +
            ((s - lake.centerDistance) / lake.radiusZ) ** 2 <
            1.04,
      );
    const pine = !coastal && !desert && (forest || city || lakes || palette.vegetation === 'pine');
    ...
    if (lakes)
      for (const geometry of lakeGeometries(start, CHUNK, settings))
        mesh(geometry, this.lakeWater.material).name = 'alpine-lake';
    if (city) group.add(this.city.buildChunk(start, CHUNK, settings));
    mesh(
      ribbon(start, CHUNK, -half - roadShoulderWidth, half + roadShoulderWidth, settings, 0),
    ...
    const count = Math.round(
      (coastal ? 30 : desert ? 20 : forest ? 90 : 60) * settings.treeDensity,
    );
    ...
    for (let i = 0; i < count; i++) {
      ...
      const offset =
        ...
        (half +
          roadShoulderWidth +
          (forest ? 2.55 : 3.25) +
          Math.pow(rng(), 1.7) * (forest ? 65 : 120));
      ...
      const height = desert
        ...
        : forest
          ? 19 + rng() * 13
          : (4.5 + rng() * 5.5) * (coastal ? 0.65 : 1);
      const rotation = rng() * Math.PI;
      // Open the downhill side between wooded bends, and leave lake surfaces clear.
      if (
        underwater(s, x, y) ||
        (city && side > 0 && (offset > half + 25 || Math.sin(s / 85) > -0.35))
      )
        continue;
      const index = treeIndex++;
      const trunkSize = forest ? 2.8 : 1;
      instance(
        trunks,
        index,
        x,
        y + height * (forest ? 0.34 : 0.25),
        -s,
        trunkSize,
        height * (forest ? 0.68 : 0.5),
        trunkSize,
        rotation,
      );
      if (pine) {
        instance(
          ...
          index,
          x,
          y + height * (forest ? 0.73 : 0.53),
          -s,
          height * (forest ? 0.27 : 0.3),
          height * (forest ? 0.55 : 0.72),
          height * (forest ? 0.27 : 0.3),
          rotation,
        );
        instance(
          ...
          index,
          x,
          y + height * (forest ? 0.9 : 0.77),
          -s,
          height * (forest ? 0.2 : 0.22),
          height * (forest ? 0.4 : 0.65),
          height * (forest ? 0.2 : 0.22),
          rotation + 0.3,
        );
      } else {
    ...
    trunks.count = crowns.count = tops.count = treeIndex;
    this.treeOcclusion.set(id, new TreeOcclusion([trunks, crowns, tops], geometries));
    ...
    for (let i = 0; i < 24; i++) {
      const s = start + rng() * CHUNK;
      const offset =
        (coastal || rng() < 0.5 ? -1 : 1) * (half + roadShoulderWidth + 3 + rng() * 52);
      const size = 0.5 + rng() * 1.9;
      const x = roadAt(s, settings).x + offset;
      const groundY = terrainAt(s, offset, settings);
      if (underwater(s, x, groundY)) continue;
      instance(
        rocks,
        rockIndex++,
        x,
    ...
    for (let i = 0; i < (coastal ? 2 : 4); i++) {
      const s = start + rng() * CHUNK;
      const offset = (coastal || city ? -1 : i % 2 ? 1 : -1) * (190 + rng() * 180);
      const height =
        (desert ? 45 + rng() * 65 : forest ? 30 + rng() * 40 : 60 + rng() * 110) *
        settings.terrainHeight;
      const mountain = new THREE.Mesh(
        desert ? this.mesaGeometry : this.mountainGeometry,
        this.materials.mountain,
      );
      mountain.name = desert ? 'sandstone-mesa' : 'distant-mountain';
      // Broad hill meshes must not cover a lake just because their centers are dry.
      if (
        lakes &&
        offset > 0 &&
        basins.some(
          (lake) =>
            Math.abs(s - lake.centerDistance) < lake.radiusZ + 180 &&
            Math.abs(roadAt(s, settings).x + offset - lake.centerX) < lake.radiusX + 160,
        )
      )
        continue;
    ...
    for (let i = 0; i < 16; i++) {
      ...
      const offset = (i % 2 ? 1 : -1) * (half + roadsideMarkerOffset);
      const p = roadAt(s, settings);
  ...
  render(
    ...
    const fallen =
      ...
      (challenge?.phase === 'gameover' && challenge.lastIncident === 'bounds');
    const recovering = challenge?.phase === 'racing' && challenge.offRoad.active;
    if ((fallen || recovering) && challenge) {
      const ground = terrainAt(state.distance, state.offset, this.settings);
      // Recovery stays driveable on the actual verge, not floating at road height.
      // Use that same surface after failure so the car cannot jump upward.
      this.car.position.y = ground - this.originHeight + 0.16;
      if (fallen)
        this.car.rotation.z = -Math.sign(state.offset) * challenge.recoveryProgress * 0.32;
    } else if (challenge?.phase === 'crashed') {
      this.car.rotation.z = Math.sin(challenge.recoveryProgress * Math.PI) * 0.1;
    }
    ...
    if (moving)
      for (const wheel of this.vehicle.wheels)
        wheel.rotation.x -= (state.speed * dt) / wheel.userData.tireRadius;
    this.vehicle.brakeLights.emissiveIntensity = braking ? 2 : 0.15;
    this.car.updateMatrixWorld(true);
    if (fallen || recovering) {
      // On a bank the uphill wheels reach the ground before the model's centre.
      // Keep the recovery pose above that support instead of burying a tire.
      let lift = 0;
      for (const wheel of this.vehicle.wheels) {
        wheel.getWorldPosition(this.wheelContact);
        const s = this.originDistance - this.wheelContact.z;
        const offset = this.wheelContact.x - roadAt(s, this.settings).x;
        const ground = terrainAt(s, offset, this.settings) - this.originHeight;
        lift = Math.max(lift, ground + wheel.userData.tireRadius + 0.04 - this.wheelContact.y);
      }
      this.car.position.y += lift;
      this.car.updateMatrixWorld(true);
    }
    ...
    // Evaluate the final framed camera, including intro, held-V and paused views.
    this.occlusionTarget.copy(this.modelCenter).applyMatrix4(this.model.matrixWorld);
    this.fadedTrees = 0;
    for (const [id, trees] of this.treeOcclusion) {
      this.fadedTrees += trees.update(
        this.chunks.get(id)!,
    ...
    const blend = this.firstFrame
      ? 1
      : 1 - Math.exp(-(this.atmosphereTransition > 0 ? 0.35 : 2.5) * dt);
    this.atmosphereTransition = Math.max(0, this.atmosphereTransition - dt);
    ...
    this.headlight.intensity = THREE.MathUtils.lerp(
      ...
      this.lighting.night
        ? 140
        : this.settings.timeOfDay === 'dusk' || this.settings.timeOfDay === 'dawn'
          ? 45
          : 0,
      blend,
    );
    ...
    this.lakeWater.update(moving ? dt : 0, this.settings);
  ...
  get environment() {
    return {
      ...
      fadedTrees: this.fadedTrees,
      trafficCars: this.traffic.count,
      trafficCache: this.traffic.telemetry,
      terrainPrefetch: this.terrainPrefetch.telemetry,
    };
  }
